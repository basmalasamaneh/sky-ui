        ...
        {isFetching ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map((skel) => (
              <div key={skel} className="bg-white rounded-3xl p-6 shadow-sm border border-[#e8dcc4] animate-pulse">
          ...
          <div className="text-center py-20 bg-white rounded-[2rem] border border-[#e8dcc4] shadow-sm">
            ...
            <h2 className="text-2xl font-bold text-[#3b2012] mb-2">لا يوجد فنانين حالياً</h2>
            <p className="text-[#9c7b65]">كن أنت أول فنان ينضم لمنصتنا من خلال إنشاء حساب فنان!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {artists.map((artist, i) => (
              ...
              >
                <Link 
                  ...
                  className="block bg-white rounded-3xl p-6 shadow-sm border border-[#e8dcc4] hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group"
                >
                  <div className="flex items-center gap-4 mb-6 relative">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-[#3b2012] group-hover:text-[#9c7b65] transition-colors flex items-center gap-2">
                        {artist.name}
                      ...
                      <p className="text-sm text-[#9c7b65] font-bold"><i className="fa-solid fa-palette ml-1 text-amber-500"></i> {artist.worksCount} أعمال فنية</p>
                    </div>
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 bg-[#f0ece6] text-[#5c4436] w-8 h-8 flex items-center justify-center rounded-full opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
                  ...
                  <div className="flex gap-2">
                    {artist.previewImages.length > 0 ? (
                      artist.previewImages.slice(0, 3).map((img, idx) => (
                        <div key={idx} className="h-24 flex-1 rounded-xl overflow-hidden relative border border-gray-100">
                          <img src={img} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                        </div>
                      ))
                    ) : (
                      <div className="h-24 w-full bg-gray-50 flex items-center justify-center rounded-xl border border-gray-100">
                    ...
                    {Array.from({ length: Math.max(0, 3 - artist.previewImages.length) }).map((_, idx) => (
                      <div key={`empty-${idx}`} className="h-24 flex-1 rounded-xl bg-[#fdfaf7] border border-[#e8dcc4]/50 flex items-center justify-center">
